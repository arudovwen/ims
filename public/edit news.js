  ...
  methods: {
    getNews: function getNews() {
      ...
      if (this.type == "news") {
        ...
        axios.get("/api/get-draft/".concat(this.$route.params.id)).then(function (res) {
          if (res.status == 200) {
            if (res.status == 200) {
              _this.post = res.data;
              _this.placeholder = res.data.cover_image;
    ...
    draft: function draft() {
      ...
      if (this.type == "news") {
        axios.post("/api/save-draft", this.post).then(function (res) {
          if (res.status == 200) {
            _this5.$toasted.info("Saved to draft");
          }
        });
      }
    },
    processUpload: function processUpload(file) {
      ...
      xhr.onload = function (progressEvent) {
        if (xhr.status === 200) {
          ...
          _this6.progress = "Completed";
          setTimeout(function () {
            _this6.start = false;
          }, 1000);
          var response = JSON.parse(xhr.response);
          _this6.uploadedFileUrl = response.secure_url; // https address of uploaded file

          _this6.post.coverImage = response.secure_url;
        } else {
          _this6.start = false;
...
var render = function() {
  ...
  return _c("div", { attrs: { id: "form" } }, [
    _c(
      ...
      [
        _c("legend", [_vm._v("Update  " + _vm._s(_vm.type))]),
        _vm._v(" "),
            ...
            _c("app-editor", {
              ...
              attrs: {
                ...
                init: {
                  ...
                  plugins:
                    "advlist autolink lists link image imagetools charmap print preview anchor insertdatetime media table paste code help wordcount lists  table  ",
                  toolbar:
